...
The confidence estimate for ExTRI pairs uses by default 2 PMIDs or 2 sentences or a score over 1.6.
  ...
  task :pairs => :tsv do |confidence,include_HTRI,ntnu_curated|
    ...
    ntnu_curated = ExTRI["NTNU_Curated." + ntnu_curated].tsv(:key_field => "Transcription Factor (Associated Gene Name)", :fields => ["Target Gene (Associated Gene Name)", "Sign", "PMID"], :merge => true, :type => :double).unzip(0,true)
