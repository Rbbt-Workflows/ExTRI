...
EOF
    ...
    ids.each do |id|
      ...
      if id != "UniProt/SwissProt Accession"
        index = Organism.identifiers(organism).tsv :key_field => "UniProt/SwissProt Accession", :fields => [id], :type => :flat, :persist => true
      ...
      if id != "Entrez Gene ID"
        index = Organism.identifiers(organism).tsv :key_field => "Entrez Gene ID", :fields => [id], :type => :flat, :persist => true
